...
export const getCourseById = async function ({
  ...
  try {
    const res = await fetch(
      `${BASE_URL}/api/courses?filters[slug][$eq]=${courseId}&locale=${locale}&populate[0]=instructor.avatar&populate[1]=sub_courses.contents&populate[2]=img&populate[3]=category&populate[4]=technologies`,
      {
        method: "GET",
        headers: {
